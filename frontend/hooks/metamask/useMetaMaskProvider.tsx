...
function useMetaMaskInternal(): UseMetaMaskState {
  const { error: eip6963Error, providers } = useEip6963() as any;
  ...
  useEffect(() => {
    ...
    if (next) {
      nextConnectListener = (connectInfo: ProviderConnectInfo) => {
        ...
        _setCurrentProvider(next); _setChainId(Number.parseInt(connectInfo.chainId, 16));
      };
      ...
      nextChainChangedListener = (chainId: string) => {
        if (next !== metaMaskProviderRef.current) { return; }
        _setCurrentProvider(next); _setChainId(Number.parseInt(chainId, 16));
      };
      chainChangedListenerRef.current = nextChainChangedListener;
      nextAccountsChangedListener = (accounts: string[]) => {
        if (next !== metaMaskProviderRef.current) { return; }
        _setCurrentProvider(next); _setAccounts(accounts);
      };
